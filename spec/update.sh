...
VERSION="4.100.0"
RELEASE="1"
CHANGELOG="KDE Frameworks 4.100.0"
...
for tier in tier{1,2,3,4}; do
  for fw in `/usr/bin/ls -1 $tier`; do
    ...
    sed -i 's/^Source0:        [0-9a-zA-Z_-\.]*/Source0:        http:\/\/download.kde.org\/unstable\/frameworks\/'"$VERSION"'\/\%{framework}-'"$VERSION"'.tar.xz/' $tier/$fw/$fw.spec
